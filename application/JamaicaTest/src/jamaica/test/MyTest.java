		...
		RealtimeThread realtimeThread = new RealtimeThread(priortyParameters,
			...
			public void run() {
				...
				for (int n = 1; n < size; n++) {
					...
					double timeInUs = (double)difference.getMilliseconds()*1000.0 + (double)difference.getNanoseconds()/1000.0;
					double period = (double)periodTime*1000.0;
					double percent = ((period-timeInUs)/period)*100.0;
					if(percent>bound || percent< (-bound)){
						System.out.println("Packet #"+n+" difference="+timeInUs+"us "+percent+"%" );
						fail++;
					}
				}
